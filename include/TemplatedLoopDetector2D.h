...
public:
   ...
   * Empty constructor
   */
  TemplatedLoopDetector2D(const Parameters &params = Parameters());
   ...
   */
  TemplatedLoopDetector2D(const TemplatedVocabulary<TDescriptor, F> &voc,
   ...
   * @param params loop detector parameters
   */
  TemplatedLoopDetector2D(const TemplatedDatabase<TDescriptor, F> &db,
    const Parameters &params = Parameters());
   ...
   */
  template<class T>
  TemplatedLoopDetector2D(const T &db, const Parameters &params = Parameters());

  /**
   * Destructor
   */
  virtual ~TemplatedLoopDetector2D(void);
...
protected:
  ...
  bool isGeometricallyConsistent_DI(EntryId old_entry, 
    ...
    const std::vector<TDescriptor> &descriptors, 
    const FeatureVector &curvec, DetectionResult &result) const;
...
template <class TDescriptor, class F> 
TemplatedLoopDetector2D<TDescriptor,F>::Parameters::Parameters():
...
template <class TDescriptor, class F> 
void TemplatedLoopDetector2D<TDescriptor,F>::Parameters::set(float f)
{
  dislocal = 20 * f;
  max_db_results = 50 * f;
...
template<class TDescriptor, class F>
TemplatedLoopDetector2D<TDescriptor,F>::TemplatedLoopDetector2D
...
template<class TDescriptor, class F>
void TemplatedLoopDetector2D<TDescriptor,F>::setVocabulary
...
template<class TDescriptor, class F>
template<class T>
void TemplatedLoopDetector2D<TDescriptor, F>::setDatabase(const T &db)
{
  delete m_database;
  m_database = new T(db);
  clear();
...
template<class TDescriptor, class F>
TemplatedLoopDetector2D<TDescriptor, F>::~TemplatedLoopDetector2D(void)
{
  delete m_database;
  m_database = NULL;
}
...
template<class TDescriptor, class F>
void TemplatedLoopDetector2D<TDescriptor,F>::allocate
...
TemplatedLoopDetector2D<TDescriptor, F>::getDatabase() const
{
  return *m_database;
}
...
TemplatedLoopDetector2D<TDescriptor, F>::getVocabulary() const
{
...
template<class TDescriptor, class F>
bool TemplatedLoopDetector2D<TDescriptor, F>::detectLoop(
  const std::vector<cv::KeyPoint> &keys, 
  const std::vector<TDescriptor> &descriptors,
  DetectionResult &match)
{
            ...
            if(getConsistentEntries() > m_params.k)
            {
              ...
              if(m_params.geom_check == GEOM_DI)
              {
                ...
                detection = isGeometricallyConsistent_DI(island.best_entry, 
                  keys, descriptors, featvec, match);
              }
...
template<class TDescriptor, class F>
inline void TemplatedLoopDetector2D<TDescriptor, F>::clear()
{
  m_database->clear();
  m_window.nentries = 0;
}
...
template<class TDescriptor, class F>
void TemplatedLoopDetector2D<TDescriptor, F>::computeIslands
...
template<class TDescriptor, class F>
double TemplatedLoopDetector2D<TDescriptor, F>::calculateIslandScore(
...
template<class TDescriptor, class F>
void TemplatedLoopDetector2D<TDescriptor, F>::updateTemporalWindow
...
template<class TDescriptor, class F>
bool TemplatedLoopDetector2D<TDescriptor, F>::isGeometricallyConsistent_DI(EntryId old_entry, const std::vector<cv::KeyPoint> &keys,
  const std::vector<TDescriptor> &descriptors,
  const FeatureVector &bowvec, DetectionResult &result) const
{
  ...
  if((int)i_old.size() >= m_params.min_Fpoints)
  {
    ...
    result.cur_points = cur_points;
    
    cv::Mat Fmat = m_fsolver.findFundamentalMat(oldMat, curMat, m_params.max_reprojection_error, result.matches, m_params.min_Fpoints,
      NULL, false, m_params.ransac_probability, m_params.max_ransac_iterations);
    return !Fmat.empty();
...
template<class TDescriptor, class F>
bool TemplatedLoopDetector2D<TDescriptor, F>::
...
template<class TDescriptor, class F>
void TemplatedLoopDetector2D<TDescriptor, F>::getFlannStructure(
  const std::vector<TDescriptor> &descriptors, 
  cv::FlannBasedMatcher &flann_structure) const
{
...
template<class TDescriptor, class F>
bool TemplatedLoopDetector2D<TDescriptor, F>::isGeometricallyConsistent_Flann
  (EntryId old_entry,
  const std::vector<cv::KeyPoint> &keys, 
  const std::vector<TDescriptor> &descriptors,
  cv::FlannBasedMatcher &flann_structure) const
...
template<class TDescriptor, class F>
void TemplatedLoopDetector2D<TDescriptor, F>::getMatches_neighratio(
...
template<class TDescriptor, class F>
void TemplatedLoopDetector2D<TDescriptor, F>::removeLowScores(QueryResults &q,
